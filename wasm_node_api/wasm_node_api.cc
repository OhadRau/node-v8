...
public:
  using type = T; /*typename
    std::conditional<is_function_pointer_v<T>,
...
void __napi_get_arraybuffer_info(
  const w::Context* ctx, const w::Val args[], w::Val results[]
) {
  napi_status status;
  ...
  void **data = (void **) &ctx->memory->data()[args[2].i32()];
  size_t *byte_length = (size_t *) &ctx->memory->data()[args[3].i32()];
  ...
  if (status == napi_ok) {
    ...
    data_addr = (char *) (data_addr - (char *) &ctx->memory->data()[0]);
    *data = (void *) data_addr;
  }

  results[0] = w::Val((int32_t) status);
}

// napi_status napi_get_buffer_info(napi_env env, napi_value buffer, void **data, size_t *length)
void __napi_get_buffer_info(
  const w::Context* ctx, const w::Val args[], w::Val results[]
) {
  napi_status status;
  ...
  napi_value arraybuffer = (napi_value) args[1].i32();
  void **data = (void **) &ctx->memory->data()[args[2].i32()];
  size_t *length = (size_t *) &ctx->memory->data()[args[3].i32()];
...
void set_napi_module_nm_register_func(
  ...
  /*
  v8::MaybeLocal<v8::Function>* funcPtr =
      new v8::MaybeLocal<v8::Function>(nm_register_func);
  // HACK: Since we can't change the underlying representation of the module,
  // we just cast our MaybeLocal pointer into a function pointer
  mod->nm_register_func = (napi_addon_register_func) funcPtr;*/
}
