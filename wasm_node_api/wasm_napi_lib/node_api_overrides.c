...
void napi_module_register(napi_module* mod) {
  opaque_ptr real_mod = create_napi_module();
  set_napi_module_nm_version(real_mod, mod->nm_version);
  set_napi_module_nm_flags(real_mod, mod->nm_flags);
  set_napi_module_nm_filename(real_mod, mod->nm_filename);
  set_napi_module_nm_register_func(real_mod, mod->nm_register_func);
  set_napi_module_nm_modname(real_mod, mod->nm_modname);
  set_napi_module_nm_priv(real_mod, mod->nm_priv);
  _napi_module_register(real_mod);
}

void finalizeBuffer(opaque_ptr env, void *finalize_data, void *finalize_hint) {
  free(finalize_data);
}

napi_status napi_create_arraybuffer(opaque_ptr env, size_t byte_length,
                                    void **data, opaque_ptr *result) {
...
napi_status napi_create_buffer(opaque_ptr env, size_t size,
                               void **data, opaque_ptr *result) {
...
napi_status napi_create_buffer_copy(opaque_ptr env, size_t length,
                                    const void *data,
                                    void **result_data,
                                    opaque_ptr *result) {
...
napi_status napi_define_properties(
  opaque_ptr env, opaque_ptr object, size_t property_count,
...
napi_status napi_define_class(
  opaque_ptr env, const char* utf8name, size_t length,
  napi_callback constructor, void* data, size_t property_count,
  const napi_property_descriptor* properties, opaque_ptr* result
) {
...
napi_status napi_get_last_error_info(
  opaque_ptr env, const napi_extended_error_info** result
) {
